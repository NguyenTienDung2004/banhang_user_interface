...
function Contact() {
    ...
    useEffect(() => {
        const getContentItemList = async () => {
            const response = await getService.getContact();
    ...
    async function handleDelete() {
        try {
            const deleteRequests = idContentItemDeleted.map((id) => deleteService.deleteContact(id));
    ...
    async function handleMarkedHeart(e, contentItem) {
        ...
        try {
            ...
            const response = await updateService.updateContact(contentItem.id, updatedContentItem);
            ...
            <div className={cx({ content_wrapper: true })}>
                ...
                <div className={cx({ list_content: true })}>
                    ...
                    <div className={cx({ list_content_body: true })}>
                        {(searchName || searchStatus ? searchResult : contentItemList).length === 0 ? (
                            <div className={cx('empty_content')}>
                                <img src={images.emptyImage} alt="empty" />
                                <p>Không tồn tại liên lạc</p>
                                <Link
                                    to={routesConfig.ContactDetail}
                                    className={cx({
                                        btn_add: true,
                            ...
                            (searchName || searchStatus ? searchResult : contentItemList).map((contentItem) => {
                                return (
                                    <Link
                                        key={contentItem.id}
